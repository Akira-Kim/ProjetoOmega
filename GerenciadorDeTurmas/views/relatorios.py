...
def build_relatorios_view(page: ft.Page) -> ft.Control:
    ...
    def on_abrir(e):
        ...
        painel.controls.extend(
            [
                ...
                ft.Text(info_aula, size=12, color=ft.Colors.GREY_400)
                if info_aula
                else ft.Container(),
                tf_titulo,
                tf_conteudo,
    ...
    def atualizar_lista(e=None):
                    ...
                    ft.Container(
                        content=ft.Column(
                            [
                                ...
                                ft.Text(
                                    f"{r.get('data_geracao') or ''}  |  turma_id={r.get('turma_id')}  |  id={r['id']}",
                                    size=11,
                                    color=ft.Colors.GREY_500,
                                ),
                            ],
        ...
        painel.controls.append(
            ft.Text(
                "Selecione um relatorio a esquerda.",
                italic=True,
                color=ft.Colors.GREY_500,
            )
        )
    ...
    # Dropdown (uma unica vez, depois das funcoes)
        # Dropdown (sem on_change no construtor)
    dd_turma = ft.Dropdown(
        label="Filtrar por turma",
        width=240,
        options=[ft.dropdown.Option(key="all", text="Todas")],
        value="all",
    )
    dd_turma.on_change = atualizar_lista

    def carregar_filtro_turmas():
        opts = [ft.dropdown.Option(key="all", text="Todas")]
    ...
    return ft.Column(
        [
            ...
            ft.Row(
                [
                    dd_turma,
                    ft.TextButton("Atualizar", on_click=atualizar_lista),
                ]
            ),
            ft.Divider(height=1),
            ft.Row(
                [
                    ft.Column([lista_col], width=320),
